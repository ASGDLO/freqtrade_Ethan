...
class FreqtradeBot(LoggingMixin):
    ...
    def exit_positions(self, trades: List[Trade]) -> int:
        ...
        for trade in trades:
            if not trade.has_open_orders and not trade.has_open_sl_orders:
                try:
                    # Handle stoploss on exchange if applicable
                    if self.strategy.order_types.get("stoploss_on_exchange"):
                        if self.handle_stoploss_on_exchange(trade):
                            trades_closed += 1
                            Trade.commit()
                            continue
                ...
                # Try to exit the trade
                if not trade.has_open_orders and trade.is_open:
                    try:
                        if self.handle_trade(trade):
                            trades_closed += 1
                        ...
                        logger.error(
                            f"Invalid order exception for trade {trade.pair}: {exception}. "
                            f"Trying to recover."
                        )
            ...
            # Sync wallets after each trade attempt to ensure the latest state
            self.wallets.update()

        # Final wallet update after all trade attempts
        if trades_closed:
            self.wallets.update()
